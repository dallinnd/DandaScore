...
function showHome() {
    ...
    app.innerHTML = `<div class="p-6 h-full flex flex-col animate-fadeIn">
        ...
        <div class="flex-1 overflow-y-auto">${list || '<p class="text-slate-600 italic">No games saved yet.</p>'}</div>
        <button onclick="startNewGame()" class="w-full bg-green-500 py-5 rounded-3xl font-black text-xl text-black mt-6 shadow-xl shadow-green-500/20">START NEW GAME</button>
    </div>`;
}
        ...
        <div class="scroll-area bg-[#0f172a]">
            ...
            <div class="p-4 space-y-3 pb-24">
                ${diceConfig.map(dice => {
                    ...
                    if (dice.id === 'blue') {
                        specialHeader = `
                        <button onclick="toggleSparkle()" class="w-full py-3 mb-2 rounded-xl font-black uppercase tracking-widest text-xs transition-all shadow-lg ${roundData.blueHasSparkle ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-500'}">
...
function setActiveInput(id) {
    ...
    activeRow.style.borderColor = config.text;
...
function updateAllDisplays() {
    ...
    diceConfig.forEach(d => {
        ...
        if (valEl) valEl.innerHTML = values.map((v, i) => `<span class="bg-black/20 px-3 py-1 rounded-lg text-sm font-black border border-black/10">
...
function updateKpDisplay() {
    ...
    if (display) display.textContent = keypadValue || (activeInputField ? `ADDING TO ${activeInputField}` : '-');
}
function kpEnter() {
...
function toggleSparkle() { 
    ...
    updateAllDisplays(); 
    renderGame(); // Refresh to update button text
}
function saveGame() {
...
function deleteGame(event, index) { event.stopPropagation(); if (confirm("Delete this game history?")) { games.splice(index, 1); localStorage.setItem('panda_games', JSON.stringify(games)); showHome(); } }
